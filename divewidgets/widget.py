...
@register
class OPTMWidget(DOMWidget, ValueWidget):
    ...
    """
    _model_name = Unicode('OPTMWidgetModel').tag(sync=True)
    _model_module = Unicode(module_name).tag(sync=True)
    _model_module_version = Unicode(module_version).tag(sync=True)
    _view_name = Unicode('OPTMWidgetView').tag(sync=True)
...
def create_optm(script="""print('Hello, World!')""", live=False, width=1100, height=700):
    if live:
        srcprefix = "https://dive4dec.github.io/optlite/live.html#code="
    else:
        srcprefix = "https://dive4dec.github.io/optlite/#mode=display&code="
    return OPTMWidget(script=script, srcprefix=srcprefix, width=width, height=height)
